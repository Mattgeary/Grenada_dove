...
hurricane <- function(template, p.trans=0.05, iterations=50, years = 35,  strike = 0.073, threshold = 0.7, total = rpois(100, 4)){
	total[total > 8] <- 8
	canes <- strike * years
	h.map <- template
...
climate.fun <- function(model, future){
	...
	env.vars <- data.frame(ppt.dry.month = as.data.frame(future.ppt.dry.month), fire = as.data.frame(fire), aspect = as.data.frame(aspect))
...
future <- function(model, years = 35, sea = TRUE, hurricane = TRUE, climate = TRUE, all = TRUE, reps=10 , threshold = 0.7){

	if(years == 35) future.ppt.dry.forest <- future.ppt.dry.forest.50
	
	if(years == 55) future.ppt.dry.forest <- future.ppt.dry.forest.70
...
# Conditional statements to add elements to the list
	...
	if(hurricane == TRUE) pred.area$hurricane <- data.frame(hurricane = numeric(reps), hurricane.low_26 = numeric(reps), hurricane.mid_45 = numeric(reps), hurricane.high_60 = numeric(reps), hurricane.max_80 = numeric(reps))

	if(all == TRUE) pred.area$all <- data.frame(hurricane = numeric(reps), sea.dev = numeric(reps), dev.hurricane = numeric(reps), sea.dev.hirricane = numeric(reps), sea.hurricane = numeric(reps), all.low_26 = numeric(reps), all.mid_45 = numeric(reps), all.high_60 = numeric(reps), all.max_80 = numeric(reps))
